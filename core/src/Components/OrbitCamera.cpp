...
void OrbitCamera::Update()
{
    ...
    if (Input::BindHeld(InputConfiguration::BIND_ORBIT_CAMERA_RESET))
    {
        scrollDelta = 0.0;
    ...
    if (Input::BindHeld(InputConfiguration::BIND_ORBIT_CAMERA_ROTATE))
    {
    ...
    if (Input::BindHeld(InputConfiguration::BIND_ORBIT_CAMERA_MOVE))
    {
        // use the height value to move faster around at higher altitude
        const float translationSpeed =
            TRANSLATION_SPEED + HEIGHT_MULTIPLIER * (std::abs(cameraPosition[2]) / HEIGHT_CEILING) * TRANSLATION_SPEED;
...
void DrawLine(Camera* camera, glm::vec3 from, glm::vec3 to, ImVec4 color, float thickness)
{
    ...
    auto viewport =
        glm::vec4(gameView->GetPosition().x, gameView->GetPosition().y,
                  gameView->GetPosition().x + gameView->GetSize().x, gameView->GetPosition().y + gameView->GetSize().y);
...
void DrawPoint(Camera* camera, glm::vec3 point, ImVec4 color)
{
    ...
    {
        ImGui::GetWindowDrawList()->AddRectFilled(screenPosition.value() - ImVec2(3, 3),
                                                  screenPosition.value() + ImVec2(3, 3),
...
void OrbitCamera::DrawOrbitPoint()
{
    ...
    DrawLine(this, orbitCameraOrigin - glm::vec3(AXIS_LENGTH, 0, 0), orbitCameraOrigin + glm::vec3(AXIS_LENGTH, 0, 0),
             ImVec4(1.0f, 0.0f, 0.0f, 0.8f), 5);
    DrawLine(this, orbitCameraOrigin - glm::vec3(0, AXIS_LENGTH, 0), orbitCameraOrigin + glm::vec3(0, AXIS_LENGTH, 0),
             ImVec4(0.0f, 1.0f, 0.0f, 0.8f), 5);
    DrawLine(this, orbitCameraOrigin - glm::vec3(0, 0, AXIS_LENGTH), orbitCameraOrigin + glm::vec3(0, 0, AXIS_LENGTH),
             ImVec4(0.0f, 0.0f, 1.0f, 0.8f), 5);
}
}  // namespace IWXMVM::Components
