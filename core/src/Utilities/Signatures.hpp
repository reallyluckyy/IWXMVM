...
inline auto DereferenceAddress = [](std::uintptr_t address) { return *reinterpret_cast<std::uintptr_t*>(address); };
                ...
                try
                {
                    ...
                    if (newAddress == 0)
                        throw std::runtime_error(
                            std::format("Failed to find correct game address (1), signature:\n\t {}", _string.data()));
