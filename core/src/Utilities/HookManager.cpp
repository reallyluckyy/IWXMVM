...
namespace IWXMVM::HookManager
{
	...
	bool WriteJumpInternal(std::uintptr_t from, std::uintptr_t to, std::size_t size = JUMP_LENGTH, std::uint8_t jumpOpcode = JUMP_OPCODE)
	{
		DWORD curProtection;
		...
		*reinterpret_cast<std::uintptr_t*>(from + 1) = relativeAddress;
		...
		if (*reinterpret_cast<uint8_t*>(from) != jumpOpcode || *reinterpret_cast<std::uintptr_t*>(from + 1) != relativeAddress)
		{
