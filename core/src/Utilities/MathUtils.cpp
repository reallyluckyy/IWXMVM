...
std::optional<ImVec2> WorldToScreenPoint(glm::vec3 point, Components::Camera& camera, glm::vec4 viewport)
{
    ...
    // this is quite the magic number, but it seems to be the scaling factor necessary to line this up with the previous
    // world to screen implementation
    const auto MAGIC_NUMBER = 0.65f;
    auto proj = glm::project(
        point, (lookat),
        glm::perspectiveFov(glm::radians(camera.GetFov() * 0.65f), viewport.z, viewport.w, 0.1f, 1000.0f), viewport);
