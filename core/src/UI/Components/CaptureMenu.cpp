...
namespace IWXMVM::UI
{
    ...
    void CaptureMenu::Render()
    {
        ...
        if (ImGui::Begin("Capture", NULL, flags))
        {
            ...
            {
                for (auto outputFormat = 0; outputFormat < Components::OutputFormat::Count; outputFormat++)
                {
                    bool isSelected = captureSettings.outputFormat == outputFormat;
                    if (ImGui::Selectable(captureManager.GetOutputFormatLabel((Components::OutputFormat)outputFormat).data(),
                                          captureSettings.outputFormat == outputFormat))
                    {
                        captureSettings.outputFormat = (Components::OutputFormat)outputFormat;
                    }
            ...
            if (ImGui::Button(ICON_FA_CIRCLE " Capture", ImVec2(ImGui::GetFontSize() * 6, ImGui::GetFontSize() * 2)))
            {
