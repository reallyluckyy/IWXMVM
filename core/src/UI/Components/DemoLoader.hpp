...
class DemoLoader : public UIComponent
{
   ...
   private:
    struct DemoDirectory
    {
        ...
        std::optional<std::size_t> parentIdx;  // Parent directory's index in the 'demoDirectories' vector. Search paths
                                               // will contain a nullopt value
    ...
    std::pair<std::size_t, std::size_t> searchPaths;  // Pair of indices representing the [first, second) interval of
                                                      // search paths in the 'demoDirectories' vector
