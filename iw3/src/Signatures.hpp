...
#define Lambda IWXMVM::Signatures::Lambdas
        ...
        Sig("8B F8 83 C7 ?? 8B 8E ?? ?? ?? ?? 3B 0D ?? ?? ?? ?? 75 ?? 83 3D ?? ?? ?? ?? 00", GAType::Code,
            26) > CG_AddPlayerSpriteDrawSurfs;  // killcam 'you' marker
        Sig("E8 ?? ?? ?? ?? 83 3D ?? ?? ?? ?? 00 74 ?? 68 ?? ?? ?? ?? E8", GAType::Code,
            -5) > CG_CalcViewValues;  // first call to AnglesToAxis we need to replace
