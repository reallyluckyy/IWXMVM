...
#define Lambda IWXMVM::Signatures::Lambdas
		...
		Sig("51 8D 90 ?? ?? ?? ?? 52 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 57",									GAType::Code, 20, Lambda::FollowCodeFlow)> fopen;
		Sig("53 8D 4C 24 ?? E8 ?? ?? ?? ?? 8D 54 24 ?? 8D 74 24 ?? 8B D8",									GAType::Code, 20, Lambda::FollowCodeFlow)> AnglesToAxis;
		...
		Sig("68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 0C 68 ?? ?? ?? ?? C1 E6 04",								GAType::Data, 1, Lambda::DereferenceAddress)> clientStatic;
