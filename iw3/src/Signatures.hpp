...
#define Lambda IWXMVM::Signatures::Lambdas
        ...
        Sig("6A 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E9 ?? 00 00 00 ?? ?? ?? ?? ?? E8", GAType::Code, 23,
            Lambda::FollowCodeFlowMinusOne) > Dvar_FindMalleableVar;
        ...
        // for rewinding
        Sig("83 C4 ?? ?? 57 56 ?? ?? ?? ?? ?? 83 C4", GAType::Code, 6, Lambda::FollowCodeFlow) > FS_Read;
