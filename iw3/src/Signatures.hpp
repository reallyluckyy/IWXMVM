...
#define Sig IWXMVM::Signatures::Signature<IWXMVM::Signatures::SignatureImpl
		...
		Sig("51 8D 90 ?? ?? ?? ?? 52 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 57",									GAType::Code, 20, FollowCodeFlow)> fopen;
		Sig("53 8D 4C 24 ?? E8 ?? ?? ?? ?? 8D 54 24 ?? 8D 74 24 ?? 8B D8",									GAType::Code, 20, FollowCodeFlow)> AnglesToAxis;
		...
		Sig("85 C0 74 ?? 8B FE E8 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? D9 41 ?? D8 4C 24 0C D9 5E 0C 5F 5E C3",	GAType::Code, -5, FollowCodeFlow)> Dvar_FindMalleable;
