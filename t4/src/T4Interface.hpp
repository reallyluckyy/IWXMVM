       ...
       public:
        ...
        void InitializeGameAddresses() final
        {
            ...
            catch (std::exception& ex)
            {
                ...
                MessageBoxA(NULL,
                            "It seems like you are running an unsupported game version.", // TODO: expand message?
        ...
        void PlayDemo(std::filesystem::path demoPath) final
        {
            ...
            const auto demoDirectory =
                std::filesystem::path(GetDvar("fs_localAppData")->value->string) / "players" / "demos";
        ...
        {
            const auto t4Dvar = Functions::FindDvar(name);

            if (!t4Dvar)
                return std::nullopt;

            Types::Dvar dvar;
            dvar.name = t4Dvar->name;
            dvar.value = (Types::Dvar::Value*)&t4Dvar->current;
